...
const AddMeeting = (props) => {
    ...
    const contactList = useSelector((state) => state?.contactData?.data)
        ...
        const createMeeting = async () => {
                setIsLoding(true)
                let result = await postApi('api/meeting/add/', {
    ...
    return (
        <Modal onClose={onClose} isOpen={isOpen} isCentered>
            ...
            <ModalContent height={"580px"}>
                ...
                <ModalFooter>
                    <Button size="sm" variant='brand' me={2} disabled={isLoding ? true : false} onClick={createMeeting}>{isLoding ? <Spinner /> : 'Save'}</Button>
                    <Button sx={{
